...
get_search_settings(S):-
	...
	setting(openlist,OSize), arg(25,S,OSize),
        ...
        ('$aleph_sat'(hovars,_HOVars) -> arg(28,S,__HOVars); arg(28,S,___HOVars)),
	...
	(setting(recursion,_Recursion) -> true; __Recursion = false),
...
rsd_clause_ok(Bottom,Clause):-
	independent_clause_ok(Bottom,Clause),
	(Bottom \= false ->
		source_sink_literals(Clause,_Body,Sources,Sinks),
		length(Sinks,N),
		N >= 1,
		connected_source_literals(Sources,___Body);
		source_sink_literals_from_modes(Clause,Sources,Sinks),
		length(Sinks,N),
		N >= 1,
		mode_connected_source_literals(Sources,__Body)).

elementary_clause_ok(Bottom,Clause):-
	independent_clause_ok(Bottom,Clause),
	(Bottom \= false ->
		source_sink_literals(Clause,_SBody,Sources,Sinks),
		length(Sinks,N),
		N == 1,
		connected_source_literals(Sources,__SBody);
		source_sink_literals_from_modes(Clause,Sources,Sinks),
		length(Sinks,N),
		N == 1,
		mode_connected_source_literals(Sources,___SBody)).
...
prove_examples(S,Flag,Contradiction,Entry,Best,CL,L2,Clause,Pos,Rand,PCover,RCover,[P,B,CL,I,G]):-
	...
	Evalfn = posonly, !,
        ...
        ((LazyOnContra = true, Contradiction = true) ->
                ...
                prove_pos(S,Flag,Entry,Best,[__PC,L2],Clause,Pos,PCover,___PC),
...
reduce(scs):-
	...
	(setting(scs_type,informed)->
		(setting(clauselength_distribution,____D) -> true;
			setting(clauselength,CL),
			estimate_clauselength_distribution(CL,100,K,_D),
			...
			p_message(1,__D),
			set(clauselength_distribution,___D));
		true),
...
reduce_prelims(L,P,N):-
	...
	('$aleph_sat'(botsize,___B) -> true;
		_B = 0, asserta('$aleph_sat'(botsize,__B))),
...
lazy_evaluate_theory([Refine|T],LazyPreds,Pos,Neg,[Refine1|T1]):-
	(Refine = A-[B,C,D,_Clause]->
        	lazy_evaluate_refinement(D,__Clause,LazyPreds,Pos,Neg,_D1,Clause1),
		Refine1 = A-[B,C,__D1,Clause1];
        	lazy_evaluate_refinement([],___Clause,LazyPreds,Pos,Neg,___D1,Refine1)),
...
old_move(theories,T):-
	...
	('$aleph_search_seen'(N,_Hash) ->
		p_message('old move'),
		true;
		asserta('$aleph_search_seen'(N,__Hash)), !,
		fail).
...
update_theory(ClauseIndex):-
	...
	index_clause(Hypothesis,ClauseIndex,Clause),
        ('$aleph_global'(example_selected,example_selected(__Type,Seed))-> true;
		_Type = pos,
                PCover = [Seed-_|_]),
